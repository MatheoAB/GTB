...
<body>
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        foreach ($_POST as $key => $value) {
            if (preg_match('/^sensor-(\d+)-id$/', $key, $matches)) {
                $index = $matches[1];
                $capteurs[$index]['id'] = $conn->real_escape_string($value);
            }
            if (preg_match('/^sensor-(\d+)-type$/', $key, $matches)) {
                $index = $matches[1];
                $capteurs[$index]['type'] = $conn->real_escape_string($value);
            }
        }

        // Vérifier si le nom du bâtiment existe déjà
        $sql_check = "SELECT COUNT(*) as count FROM batiment WHERE nom = '$nom'";
        $result_check = $conn->query($sql_check);

        if ($result_check) {
            $row = $result_check->fetch_assoc();
            if ($row['count'] > 0) {
                echo "<div class='error'>Un bâtiment avec ce nom existe déjà. Veuillez choisir un autre nom.</div>";
            } else {
                ...
                if ($conn->query($sql) === TRUE) {
                    ...
                    foreach ($capteurs as $capteur) {
                        $sql_capteur = "INSERT INTO capteurs (batiment_id, identifiant_lorawan, type) VALUES ('$batiment_id', '{$capteur['id']}', '{$capteur['type']}')";
            ...
            echo "<div class='error'>Erreur lors de la vérification du nom : " . $conn->error . "</div>";
        }
            ...
            if (numSensors && numSensors > 0) {
                for (let i = 1; i <= numSensors; i++) {
                    ...
                    sensorGroup.innerHTML = `
                        <label for="sensor-${i}-id">Identifiant LoRaWAN (Capteur ${i})</label>
                        <input type="text" name="sensor-${i}-id" id="sensor-${i}-id" placeholder="Identifiant LoRaWAN" required>
                        <label for="sensor-${i}-type">Type de Capteur ${i}</label>
                        <select name="sensor-${i}-type" id="sensor-${i}-type" required>
